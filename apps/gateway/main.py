...
async def proxy_request(service_url: str, path: str, request: Request):
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=300.0) as client:
        try:
            ...
            return JSONResponse(
                content={"error": f"Service unavailable: {exc}", "url": url},
                status_code=503,
            )
...
async def service_health_proxy(service_name: str, request: Request):
    ...
    if service_name not in SERVICES:
        return JSONResponse(
            content={"error": f"Unknown service: {service_name}"}, status_code=404
        )
...
# Auth Routes
@app.api_route(
    "/api/auth/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"]
)
...
# Organization Routes
@app.api_route(
    "/api/organization/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"]
)
async def org_proxy(path: str, request: Request):
    return await proxy_request(
        SERVICES["organization"], f"/api/organization/{path}", request
    )
...
@app.api_route(
    "/api/locations/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"]
)
async def location_proxy(path: str, request: Request):
    return await proxy_request(
        SERVICES["organization"], f"/api/locations/{path}", request
    )
...
# Inventory Routes
@app.api_route(
    "/api/inventory/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"]
)
...
# Suppliers Routes
@app.api_route(
    "/api/suppliers/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"]
)
...
# Team Routes
@app.api_route(
    "/api/team/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"]
)
...
# Orders Routes — not implemented yet (no orders service)
@app.api_route("/api/orders", methods=["GET", "POST"])
@app.api_route(
    "/api/orders/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"]
)
async def orders_proxy(request: Request):
    from fastapi.responses import JSONResponse

    return JSONResponse(
        content={"error": "Orders service not implemented"}, status_code=501
    )


# Shipments Routes
@app.api_route(
    "/api/shipments/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"]
)
